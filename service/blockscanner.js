...
function* saveBlockRange(channelName,start,end){
    while(start<end){
        ...
        for(let i=0;i<txLen;i++){
            ...
            yield sql.updateBySql(`update chaincodes set txcount =txcount+1 where  channelname='${channelName}' `)
        }
...
function* saveChaincodes(channelName){
    let chaincodes=yield query.getInstalledChaincodes('peer1',channelName,'instantiated','admin','org1')
    ...
    for(let i=0;i<len;i++){
        ...
        let c= yield sql.getRowByPkOne(`select count(1) as c from transaction where channelname='${channelName}'`)
        if(c.c==0){
